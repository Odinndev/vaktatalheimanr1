    ...
    <div class="uk-card-header">
        ...
        <div class="uk-grid-small uk-flex-middle" uk-grid>
            ...
            <div class="uk-width-auto" v-if="post.hver == 'odinn'">
                <img class="uk-border-square" width="25" height="25" src="https://lh3.googleusercontent.com/-RNjux6ykggw/XLMZ9bnbSZI/AAAAAAAA6D0/rC_x-24Z-8IWZNNooCnbQeSQtp9mWYJ0wCEwYBhgL/w139-h140-p/IMG_1476.JPG">
            </div>
             <div class="uk-width-auto" v-if="post.hver == 'heida'">
                <img class="uk-border-square" width="25" height="25" src="https://scontent.frkv2-1.fna.fbcdn.net/v/t1.0-1/c0.0.100.100a/p100x100/74287561_10156237732471152_2742321869578305536_n.jpg?_nc_cat=110&_nc_oc=AQlY54n8aWfGKdvuWtnmtbD2jm_QHJXggU1VSJdfpEt4lsse41JMcXN4f63Bsdwq5QQ&_nc_ht=scontent.frkv2-1.fna&oh=04c53587f4712aeafe87bf05d28a7601&oe=5E51BC03">
            </div>
            ...
            <div class="uk-width-expand">
               <!-- ODINN -->
                ...
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="timi" uk-icon="icon: clock"></span><span class="time">{{ post.dagsetning | moment("H:mm") }}</span>
                <span class="vakt">Seinni Vakt</span></time></p>

                <p class="uk-text-meta uk-margin-remove-top">
                <time class="helgivakt" v-if="post.vaktir == 'helgivakt'">
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="timi" uk-icon="icon: clock"></span><span class="time">{{ post.dagsetning | moment("H:mm") }}</span>
                <span class="vakt">Helgarvakt</span></time></p>

                <p class="uk-text-meta uk-margin-remove-top">
                <time class="varmahlid" v-if="post.vaktir == 'varmahlid'">
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="timi" uk-icon="icon: clock"></span><span class="time">{{ post.dagsetning | moment("H:mm") }}</span>
                <span class="vakt">Varmahlíð</span></time></p>

                <p class="uk-text-meta uk-margin-remove-top">
                <time class="fri" v-if="post.vaktir == 'fri'">
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="vakt">Frídagur</span></time></p>
                ...
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="timi" uk-icon="icon: clock"></span><span class="time">{{ post.dagsetning | moment("H:mm") }}</span>
                <span class="vakt">Kvöldvakt</span></time></p>

                <p class="uk-text-meta uk-margin-remove-top">
                <time class="morgun" v-if="post.vaktir == 'morgunvakt'">
                <span class="cal" uk-icon="icon: calendar"></span><span class="date">{{ post.dagsetning | moment('timezone', 'Atlantic/Reykjavik', 'ddd ll') }}</span> 
                <span class="timi" uk-icon="icon: clock"></span><span class="time">{{ post.dagsetning | moment("H:mm") }}</span>
                <span class="vakt">Morgunvakt</span></time></p>
